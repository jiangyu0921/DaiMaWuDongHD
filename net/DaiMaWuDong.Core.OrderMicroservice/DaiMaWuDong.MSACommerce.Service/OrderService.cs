...
namespace DaiMaWuDong.MSACommerce.Service
{
    public class OrderService : IOrderService
    {
        ...
        public long CreateOrder(OrderDto orderDto, UserInfo userInfo)
        {
            ...
            #region 数据准备
            //生成订单ID，采用自己的算法生成订单ID [考虑并发问题？需要分布式锁]
            ...
            try
            {
                ...
                #region 本地数据库 数据一致性只需要进行事务包裹就可以
                // 开启事务
            ...
            #region 异步清理购物车+延时取消任务+确认订单支付状态任务
            {
                ...
                try
                {
                    ...
                    //发布清理购物车任务
                    _RabbitMQInvoker.Send(RabbitMQExchangeQueueName.OrderCreate_Exchange, message);
                    _RabbitMQInvoker.Send(RabbitMQExchangeQueueName.OrderPay_Exchange, message);

                    this._RabbitMQInvoker.SendDelay(RabbitMQExchangeQueueName.OrderCreate_Delay_Exchange, message, 60 * 30);
                    //发布延时关闭订单任务
                    //_RabbitMQInvoker.SendDelay(RabbitMQExchangeQueueName.OrderCreate_Delay_Exchange, message, 30);

                }
                catch (Exception e)
                {
                    _logger.LogError($"发送异步购物车清理消息失败，OrderId={order.OrderId}，UserId={userInfo.id}");
                }
            }
            #endregion
        ...
        {
            var orderStatus = _orangeContext.TbOrderStatus.Find(orderId);
            if (orderStatus == null)
        ...
        public bool CloseOrder(long orderId)
        {
            ...
            var skuIdList = _orangeContext.TbOrderDetail.Where(od => od.OrderId == orderId).Select(od => new
            {
                skuId = od.SkuId,
                num = od.Num
            }).ToList();
            ...
            {
                #region 本地数据库
                //trans = _orangeContext.Database.BeginTransaction();
                ...
                this._orangeContext.SaveChanges();
                Console.WriteLine("关闭订单===>数据库业务数据已经插入,操作完成");
                trans.Commit();
                #endregion
            }
            catch (Exception ex)
